// bootp-fuzz-template.cc
//
// Singular, self-contained fuzz harness for BootpServiceDetector.
// Pedagogical companion to bootp_validate_fuzz.cc + appid_fuzz_api.cc:
// the same setup pattern, but specialized to BOOTP and inlined directly
// into LLVMFuzzerTestOneInput so the entire harness is one function.
