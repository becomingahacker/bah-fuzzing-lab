...
found a real heap buffer overflow in a proprietary MIME decoder.

## What You'll Learn

1. How to pick a fuzzing target inside extracted firmware
2. How to find a function's prototype using radare2
...
- AFL++ built from source with QEMU mode for i386 (`CPU_TARGET=i386`)
- The Sophos SFOS root filesystem extracted and available (`$ROOT`)
- Python `lief` package (`pip3 install lief`)
- `radare2` for disassembly
- 32-bit clang toolchain (`apt install clang`)
- Basic C and x86 assembly knowledge

## Files

The lessons start off pretty dense to make sure you have all the info you need, and then they get more to the point
as we get going.
...
fuzzer_lesson/
  ...
  02_reverse_engineering.md -- disassemble the API with r2
  03_writing_harness.md  -- LIEF conversion, stubs, the C harness
  04_running_afl.md      -- seeds, QEMU mode, QASan, CMPLOG, the crash
  05_exercise.md         -- your turn: fuzz a different parser
  harness/               -- working code (setup.sh builds everything)
  exercise/              -- skeleton with TODOs for you to fill in
```

Make sure to ask questions if you get stuck!  During the workshop we'll have Breakout Rooms on WebEx we can use to 1:1 debug things,
and during the conference we'll be around to answer questions.
